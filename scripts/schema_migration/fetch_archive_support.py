...
parser.add_argument("--steps", choices=['mod_schema', 'mod_data', 'both'],
                    default='mod_schema')
parser.add_argument("--perm-only", action="store_true",
                    help="Only grant permissions for archive table")
...
if args.steps in ['mod_schema', 'both']:
    ...
    if not args.perm_only:
        arch_cols = ["archive_id integer primary key constraint archive_primary  generated by default as identity",
    ...
    if not args.perm_only and not args.arch_only:
    ...
    for acct in ["reg_reader", "reg_writer"]:
        try:
            with db_connection.engine.connect() as conn:
                if (acct == "reg_reader" or args.schema_type == "production") and (not args.no_permission_restrictions):
                    privs = "SELECT"
                else:
                    privs = "SELECT, INSERT, UPDATE"
                select_prv = (
                    f"GRANT {privs} ON TABLE {schema}.archive TO {acct}"
                )
                conn.execute(text(select_prv))

                # Need select access to sequences to create entries
                if (
                    acct == "reg_writer" and args.schema_type != "prodcution"
