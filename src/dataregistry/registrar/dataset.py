...
class DatasetTable(BaseTable):
    ...
    def _validate_register_inputs(
        ...
        if not self.root_dir_exists:
            raise FileNotFoundError(f"root_dir {self._root_dir} doesn't exist")
        ...
        for i_char in _ILLEGAL_NAME_CHAR:
            if i_char in name:
                raise ValueError(f"Illegal character {i_char} in name string")
        ...
        if "relative_path" in kwargs_dict.keys():
            if not kwargs_dict["relative_path"] is None:
                ...
                # Make sure `relative_path` is legal (i.e., no illegal
                # characters)
                ...
                # '.gen_paths' directory is reserved for autogenerated paths
        ...
        if kwargs_dict["owner_type"] not in self._OWNER_TYPES:
            raise ValueError(f"Invalid owner_type {kwargs_dict['owner_type']}")
    ...
    def _compute_version_string(self, name, version, kwargs_dict):
        ...
        if version not in ["major", "minor", "patch"]:
            ...
            v_fields = _bump_version(
                name,
                version,
                self._get_table_metadata("dataset"),
                self._engine
            )
            version_string = (
    ...
    def _extract_kwargs_to_dict(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            # Check for missing arguments, use their default values if present
    ...
    def _register_row(self, name, version, kwargs_dict, gen_path=False):
        ...
        # will be upgraded to valid if data copying (if any) was successful.
        kwargs_dict["status"] = 0

        # Create a new row in the data registry database.
        dataset_table = self._get_table_metadata("dataset")
        with self._engine.connect() as conn:
            prim_key = add_table_row(conn,
                                     dataset_table,
                                     kwargs_dict,
                                     commit=True)
    ...
    def register(
        ...
        input_production_datasets : list, optional
            List of production dataset ids that were inputs to this execution
        ...
        if kwargs_dict["location_type"] in ["dataregistry", "dummy"]:
            ...
            if len(previous_datasets) > 0:
                ...
                if not get_dataset_status(previous_datasets[-1].status,
                                          "deleted"):
                    if will_copy:
                        raise ValueError(
    ...
    def replace(
        ...
        if kwargs_dict["location_type"] in ["dataregistry", "dummy"]:
            ...
            if get_dataset_status(previous_datasets[-1].status, "archived"):
                msg = f"Dataset {full_name} is archived, cannot replace"
                raise ValueError(msg)
            # Cannot replace deleted datasets
            if get_dataset_status(previous_datasets[-1].status, "deleted"):
                msg = f"Dataset {full_name} is deleted, cannot replace"
                raise ValueError(msg)
            ...
            if not self._check_write_permission(previous_datasets[-1].dataset_id):
                msg = f"Dataset {full_name} is owned by another user, ",
                msg += "cannot replace"
                raise ValueError(msg)
    ...
    def _handle_data(self, relative_path, old_location, owner, owner_type,
        ...
        if old_location:
            ...
            _INFRA_UMASK = 0o007
            old_mask = os.umask(_INFRA_UMASK)
    ...
    def delete(self, name, version_string, owner, owner_type, confirm=False):
        ...
        if not self._check_write_permission(the_id):
            msg = f"Dataset {name}, {version_string} owned by other user.  "
            msg += "Cannot delete."
            raise ValueError(msg)
    ...
    def _delete_by_id(self, dataset_id, confirm=False):
        ...
        if get_dataset_status(previous_dataset.status, "deleted"):
            raise ValueError(f"Dataset {dataset_id} previously deleted")
        ...
        with self._engine.connect() as conn:
            update_stmt = (
                ...
                .values(
                    status=set_dataset_status(previous_dataset.status,
                                              deleted=True),
        ...
        msg = f"Deleted {dataset_id} from data registry"
        self.db_connection.logger.info(msg)
